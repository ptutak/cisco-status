...
from cisco_status.resolver import RouterHSRPResolver, Router, DesiredHSRPConfig
...
def test_resolver():
    resolver = RouterHSRPResolver("CE1", MyRouter(), [
    ...
    assert resolver.resolve_router_config() == {'CE1': {'Gi0/0/1': [{'group': 'Group 1', 'status': 'Pass'}, {'group': 'Group 2', 'status': 'Pass'}]}}
...
def test_resolver_fail():
    ...
    assert resolver.resolve_router_config() == {'CE1': {'Gi0/0/1': [{'group': 'Group 1', 'status': 'Fail - No longer Standby'}, {'group': 'Group 2', 'status': 'Pass'}]}}
